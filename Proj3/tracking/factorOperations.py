...
def joinFactors(factors: List[Factor]):
    ...
    conditionedVariables = (
        functools.reduce(lambda x, y: x | y, setsOfConditioned) - unconditionedVariables
    )
    ...
    for assignmentDict in joined.getAllPossibleAssignmentDicts():
        probability = functools.reduce(
            lambda x, y: x * y,
            [factor.getProbability(assignmentDict) for factor in factors],
        )
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignmentDict in factor.getAllPossibleAssignmentDicts():
            eliminated.setProbability(
                assignmentDict,
                eliminated.getProbability(assignmentDict)
                + factor.getProbability(assignmentDict),
            )
        return eliminated
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
